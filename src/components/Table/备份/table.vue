...
<script lang="ts">
import {defineComponent, ref, computed, nextTick, PropType, watch, provide, onMounted} from 'vue';
...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const head = ref<WrapWithUndefined<HTMLElement>>(undefined);
    ...
    const setSeparateHeight = () => {
      nextTick(() => {
        ...
        if (head.value && body) {
          ...
          separateHeight.value = props.maxHeight > 0 && body.clientHeight - head.value.clientHeight > props.maxHeight;
        }
      });
    }
    ...
    const bodyStyle = computed(() => { // 这里本是方法
      ...
      if (separateHeight.value) {
        ...
        result.maxHeight = props.maxHeight - (head.value?.clientHeight || 0) + 'px';
      }
      if (props.selectMode) {
    ...
    const handleBodyScroll = (event: Event) => {
      ...
      if (direction === 'horizontal') {
        ...
        if (head.value) {
          ...
          ScrollServe.setScroll(head.value, target.scrollLeft, false);
        }
